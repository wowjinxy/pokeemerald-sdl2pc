...
{
    bool8 (*addr)(void) = (bool8 (*)(void))ScriptReadQuadWord(ctx);
...
{
    NativeFunc func = (NativeFunc)ScriptReadQuadWord(ctx);
...
{
    const u8 *ptr = (const u8 *)ScriptReadQuadWord(ctx);
...
bool8 ScrCmd_setvaddress(struct ScriptContext *ctx)
{
    ...
    u32 addr2 = ScriptReadQuadWord(ctx);
...
{
    u32 addr = ScriptReadQuadWord(ctx);
...
{
    u8 condition = ScriptReadByte(ctx);
    const u8 *ptr = (const u8 *)(ScriptReadQuadWord(ctx) - sAddressOffset);
...
bool8 ScrCmd_loadbytefromptr(struct ScriptContext *ctx)
{
    ...
    ctx->data[index] = *(const u8 *)ScriptReadQuadWord(ctx);
    return FALSE;
}
...
bool8 ScrCmd_setptr(struct ScriptContext *ctx)
{
    ...
    *(u8 *)ScriptReadQuadWord(ctx) = value;
    return FALSE;
}
...
bool8 ScrCmd_setptrbyte(struct ScriptContext *ctx)
{
    ...
    *(u8 *)ScriptReadQuadWord(ctx) = ctx->data[index];
    return FALSE;
}
...
{
    u8 *ptr = (u8 *)ScriptReadQuadWord(ctx);
    *ptr = *(const u8 *)ScriptReadQuadWord(ctx);
    return FALSE;
}
...
{
    const u8 value1 = ctx->data[ScriptReadByte(ctx)];
    const u8 value2 = *(const u8 *)ScriptReadQuadWord(ctx);
...
{
    const u8 value1 = *(const u8 *)ScriptReadQuadWord(ctx);
    const u8 value2 = ctx->data[ScriptReadByte(ctx)];
...
{
    const u8 *msg = (const u8 *)ScriptReadQuadWord(ctx);
...
{
    u64 msg = ScriptReadQuadWord(ctx);
...
{
    u8 stringVarIndex = ScriptReadByte(ctx);
    const u8 *text = (u8 *)ScriptReadQuadWord(ctx);
...
{
    const u8 *ptr = (u8 *)(ScriptReadQuadWord(ctx) - sAddressOffset);
...
{
    u8 stringVarIndex = ScriptReadByte(ctx);
    u64 addr = ScriptReadQuadWord(ctx);
...
{
    const void *ptr = (void *)ScriptReadQuadWord(ctx);
