...
static bool write_tree(unsigned char * dest, HuffNode_t * tree, int nitems, struct BitEncoding * encoding) {
    /*
    ...
    if (traversal == NULL) {
        fprintf(stderr, "Fatal error while compressing Huff file.\n");
        return false;
    }
        ...
        for (j = 0; i < 2 * nitems - 1 && j < 1 << depth; j++) {
            ...
            if (k == depth) {
                ...
                if (traversal + i - parent > 128) {
                    fprintf(stderr, "Fatal error while compressing Huff file: unable to encode binary tree.\n");
                    return false;
                }
...
unsigned char * HuffCompress(unsigned char * src, int srcSize, int * compressedSize_p, int bitDepth) {
    ...
    bool did_write = write_tree(dest, freqs, nitems, encoding);
...
fail:
    free(dest);
    fprintf(stderr, "Fatal error while decompressing Huff file.\n");
    return NULL;
}
